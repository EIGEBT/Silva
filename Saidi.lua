...
SASAII = Redis:sismember(Saidi.."Saidi:SASAII:Groups",UserId) ,
...
elseif Statusrestricted(msg.chat_id,msg.sender.user_id).SASAII == true then
...
local ban = LuaTele.getUser(msg.sender.user_id)
local photo = LuaTele.getUserProfilePhotos(msg.sender.user_id)
local news = ' اسمك -› '..UserInfo.first_name
if photo.total_count > 0 then
data = {} 
data.inline_keyboard = {
{
...
if not Redis:sismember(Saidi.."Saidi:MteaU:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," ⦁ تم تنزيله من المتوحدين مسبقا ").Reply,"md",true)  
else
Redis:srem(Saidi.."Saidi:MteaU:Group"..msg_chat_id,Message_Reply.sender.user_id) 
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id," ⦁ تم تنزيله من قائمه المتوحدين ").Reply,"md",true)  
end
end
...
if not msg.Originators and not Redis:get(Saidi.."Saidi:JabwA:Group"..msg_chat_id) then
return LuaTele.sendText(msg_chat_id,msg_id,"* ⦁ تم تعطيل اوامر رفع التسليه *","md",true)
end 
if Redis:sismember(Saidi.."Saidi:MteaU:Group"..msg_chat_id,Message_Reply.sender.user_id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id,"* ⦁ تم رفع العضو متوحد مسبقا *").Reply,"md",true)  
else
Redis:sadd(Saidi.."Saidi:MteaU:Group"..msg_chat_id,Message_Reply.sender.user_id) 
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(Message_Reply.sender.user_id,"* ⦁ تم رفع العضو متوحد في الجروب *").Reply,"md",true)  
end
end
if TextMsg == "مميز" then
if not msg.Addictive then
...
local Info_Members = Redis:smembers(Saidi.."Saidi:SASAII:Groups") 
if #Info_Members == 0 then
...
local reply_markup = LuaTele.replyMarkup{
type = 'inline',
data = {{{text = '𓄼• مسح المكتومين عام •𓄹', data = msg.sender.user_id..'/SASAII'},},}}
return LuaTele.sendText(msg_chat_id, msg_id, ListMembers, 'md', false, false, false, false, reply_markup)
end
...
Redis:set(Saidi.."Saidi:JabwA:SetId"..msg_chat_id,true) 
return LuaTele.sendText(msg_chat_id,msg_id,"* ⦁ تم تفعيل اومر التسليه *","md",true)
end
...
Redis:del(Saidi.."Saidi:JabwA:SetId"..msg_chat_id) 
...
if Redis:sismember(Saidi.."Saidi:SASAII:Groups",UserId_Info.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(UserId_Info.id," ⦁ تم كتمه عام من المجموعات مسبقا ").Reply,"md",true)  
else
Redis:sadd(Saidi.."Saidi:SASAII:Groups",UserId_Info.id) 
...
if not Redis:sismember(Saidi.."Saidi:SASAII:Groups",UserId_Info.id) then
...
Redis:srem(Saidi.."Saidi:SASAII:Groups",UserId_Info.id) 
...
if Redis:sismember(Saidi.."Saidi:SASAII:Groups",Message_Reply.sender.user_id) then
...
Redis:sadd(Saidi.."Saidi:SASAII:Groups",Message_Reply.sender.user_id) 
...
if not Redis:sismember(Saidi.."Saidi:SASAII:Groups",Message_Reply.sender.user_id) then
...
Redis:srem(Saidi.."Saidi:SASAII:Groups",Message_Reply.sender.user_id) 
...
Redis:del(Saidi.."Saidi:SASAII:Groups") 
...
{
{text = '𓄼• ᴊᴀʙᴡᴀ •𓄹', url = 't.me/J_A_B_W_A'},{text = '𓄼• ᴊᴇᴋᴀ •𓄹', url = 't.me/Dev_Jeka'}, 
},
{
{text = '𓄼• ᴘʀᴏɢʀᴀᴍᴍᴇʀѕ ѕᴀɪᴅɪ •𓄹', url = 't.me/DevSaidi'}, 
},
{
{text = 'أضغط لاضافه ألبوت لمجموعتك 𖠪', url = 't.me/'..UserBot..'?startgroup=new'},
},
...
elseif Text and Text:match('(%d+)/SASAII') then
local UserId = Text:match('(%d+)/SASAII')
if tonumber(IdUser) == tonumber(UserId) then
Redis:del(Saidi.."Saidi:SASAII:Groups") 
